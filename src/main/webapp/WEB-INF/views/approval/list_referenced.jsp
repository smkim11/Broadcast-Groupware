                        ...
                        <tbody>
                            <c:forEach var="doc" items="${docs}">
                                <tr>
                                    ...
                                    <td>${empty doc.fullName ? loginUser.fullName : doc.fullName}</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${not empty doc.updateDate}">
                                                ${fn:substring(doc.updateDate, 0, 10)}
                                            </c:when>
                                            <c:otherwise>-</c:otherwise>
                                        </c:choose>
                                    </td>
                            ...
                            <c:if test="${empty docs}">
                                <tr>
                                    <td colspan="5" class="text-center text-muted">참조된 문서가 없습니다.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
