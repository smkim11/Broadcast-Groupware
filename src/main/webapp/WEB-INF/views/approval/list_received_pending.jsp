...
<head>
<meta charset="UTF-8">
<title>List Received Pending</title>
</head>
<body>
<div>
                    ...
                    <table class="table table-hover align-middle mb-0 text-center">
                        ...
                        <tbody>
                            <c:forEach var="doc" items="${docs}">
                                <tr>
                                    <td>
                                        <fmt:formatNumber value="${doc.approvalDocumentId}" type="number" minIntegerDigits="6" groupingUsed="false"/>
                                    </td>
                                    <td class="text-start">
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${not empty doc.createDate}">${fn:substring(doc.createDate, 0, 10)}</c:when>
                                            <c:otherwise>-</c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
                            ...
                            <c:if test="${empty docs}">
                                <tr>
                                    <td colspan="5" class="text-muted">결재 대기 문서가 없습니다.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
